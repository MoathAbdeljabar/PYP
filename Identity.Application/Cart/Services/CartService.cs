...
public class CartService : ICartService
{
    ...
    public async Task<ServiceResult<object>> RemoveCartItemAsync(string userId, int cartItemId)
    {
        var cartItem = await _context.CartProducts.FindAsync(cartItemId);

        if (cartItem == null)
        {
            return ServiceResult<object>.Failure(BusinessErrorType.ResourceNotFound, "Cart Item Not Found");
        }
        ...
        if (cart == null)
        {
            return ServiceResult<object>.Failure(BusinessErrorType.ResourceNotFound, "Cart Not Found");
        }

        if (cart.Id != cartItem.CartId)
        {
            return ServiceResult<object>.Failure(BusinessErrorType.OperationNotAllowed, "That item is not in your cart");
        }
        ...
        try
        {
            ...
            await _context.SaveChangesAsync();
            return ServiceResult<object>.Success(null, "Cart item deleted successfully");
        }
        catch (Exception ex)
        {
